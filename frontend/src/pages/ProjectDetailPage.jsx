...
export default function ProjectDetailPage({ project: p, refreshProjects, colors, navigate }) {
  ...
  const [historyModal, setHistoryModal] = useState(null); // step data
  ...
  const startEditStep = (phaseId, step) => {
    setEditingStep({ phaseId, stepId: step.id });
    setEditForm({ planned: step.planned || '', actual: step.actual || '', owner: step.owner || '' });
  };

  const saveStepEdit = async () => {
    if (!editingStep) return;
    try {
      await apiUpdateStep(p.id, editingStep.phaseId, editingStep.stepId, {
        ...editForm,
        changed_by: user?.name || user?.email || 'Unknown',
      });
  ...
  return (
    <div data-testid="project-detail-page">
      ...
      <div className="sticky top-0 z-10 bg-neutral-950/80 backdrop-blur-xl border-b border-neutral-800 px-8 py-5">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex gap-2 shrink-0">
            <button data-testid="export-csv-btn" onClick={handleExport} disabled={exporting} className="inline-flex items-center gap-1.5 rounded-md text-sm font-medium border border-neutral-800 bg-transparent hover:bg-neutral-800 text-white h-9 px-3 transition-colors disabled:opacity-50">
                        ...
                        <table className="w-full text-sm min-w-[700px]">
                          <thead>
                            <tr className="border-b border-neutral-800">
                              ...
                              <th className="text-[10px] font-bold tracking-[0.1em] uppercase text-neutral-500 px-3 py-2 text-left w-24">Owner</th>
                              <th className="text-[10px] font-bold tracking-[0.1em] uppercase text-neutral-500 px-3 py-2 text-left w-28">Planned</th>
                              <th className="text-[10px] font-bold tracking-[0.1em] uppercase text-neutral-500 px-3 py-2 text-left w-28">Actual</th>
                              <th className="text-[10px] font-bold tracking-[0.1em] uppercase text-neutral-500 px-3 py-2 text-left w-28">Status</th>
                              <th className="text-[10px] font-bold tracking-[0.1em] uppercase text-neutral-500 px-3 py-2 text-center w-16">Revisions</th>
                              <th className="text-[10px] font-bold tracking-[0.1em] uppercase text-neutral-500 px-2 py-2 text-center w-12"></th>
                            </tr>
                          </thead>
                          <tbody>
                            {(ph.steps || []).map(s => {
                              ...
                              return (
                                <tr key={s.id} className={`border-b border-neutral-800/50 hover:bg-neutral-800/30 ${s.critical ? 'border-l-2 border-l-amber-400/40' : ''}`} data-testid={`step-${s.id}`}>
                                  ...
                                  <td className="px-3 py-2.5">
                                    {isEditing ? (
                                      <input value={editForm.owner} onChange={e => setEditForm(f => ({ ...f, owner: e.target.value }))} className="w-full h-6 bg-neutral-950 border border-neutral-700 rounded px-1.5 text-[11px] text-white focus:outline-none focus:ring-1 focus:ring-neutral-600" />
                                    ) : (
                                      <span className="text-xs text-neutral-400 cursor-pointer hover:text-white" onClick={() => startEditStep(ph.id, s)}>{s.owner || '—'}</span>
                                    )}
                                  </td>
                                  <td className="px-3 py-2.5">
                                    {isEditing ? (
                                  ...
                                  <td className="px-3 py-2.5 text-center">
                                    {revCount > 0 ? (
                                      <button data-testid={`history-btn-${s.id}`} onClick={() => setHistoryModal(s)} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-amber-400/10 text-amber-400 border border-amber-400/20 hover:bg-amber-400/20 transition-colors">
                                        <History className="w-3 h-3" />{revCount}
                                      </button>
                                    ) : (
                                  ...
                                  <td className="px-2 py-2.5 text-center">
                                    {isEditing ? (
                                      <div className="flex gap-1">
                                        <button data-testid={`save-step-${s.id}`} onClick={saveStepEdit} className="text-emerald-400 hover:text-emerald-300 p-0.5"><Save className="w-3.5 h-3.5" /></button>
                                        <button onClick={() => setEditingStep(null)} className="text-neutral-500 hover:text-white p-0.5"><X className="w-3.5 h-3.5" /></button>
                                      </div>
                                    ) : (
                                      <button data-testid={`edit-step-${s.id}`} onClick={() => startEditStep(ph.id, s)} className="text-neutral-600 hover:text-white transition-colors p-0.5">
                            ...
                            {(!ph.steps || ph.steps.length === 0) && (
                              <tr><td colSpan={7} className="text-center text-neutral-600 text-xs py-4">No steps</td></tr>
                            )}
                          </tbody>
                        </table>
                      </div>
                ...
                return (
                  <div key={team} className="bg-neutral-900 border border-neutral-800 rounded-md p-4" data-testid={`team-card-${team}`}>
                    ...
                    <div className="space-y-1">
                      {data.phases.map(phh => <div key={phh} className="text-[11px] text-neutral-500">- {phh}</div>)}
                    </div>
                  </div>
                );
              })}
          ...
          <TabsContent value="packaging">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6" data-testid="packaging-info">
              <div className={`bg-neutral-900 border rounded-md p-4 ${p.pt === 'New' ? 'border-l-2 border-l-purple-400 border-neutral-800' : 'border-l-2 border-l-blue-400 border-neutral-800'}`}>
                ...
                <div className={`text-[11px] mt-1 ${p.pt === 'New' ? 'text-purple-400' : 'text-blue-400'}`}>{p.pt === 'New' ? 'New tooling required' : 'Using existing mold'}</div>
              </div>
      ...
      <Dialog open={!!historyModal} onOpenChange={() => setHistoryModal(null)}>
        <DialogContent className="bg-neutral-900 border-neutral-800 max-w-lg text-white" data-testid="date-history-modal">
          <DialogHeader>
            <DialogTitle className="text-base font-semibold text-white font-display">Date Change History</DialogTitle>
            <DialogDescription className="text-neutral-400 text-sm">{historyModal?.step}</DialogDescription>
          </DialogHeader>
          <div className="space-y-2 max-h-[60vh] overflow-y-auto">
            {(historyModal?.date_history || []).length === 0 && (
              <div className="text-sm text-neutral-500 text-center py-4">No date changes recorded</div>
            )}
            {(historyModal?.date_history || []).sort((a, b) => (b.changed_at || '').localeCompare(a.changed_at || '')).map((h, i) => (
              <div key={i} className="bg-neutral-950 border border-neutral-800 rounded-md p-3" data-testid={`history-entry-${i}`}>
                <div className="flex items-center gap-2 mb-1">
                  <Badge className={`text-[10px] ${h.field === 'planned' ? 'bg-blue-400/10 text-blue-400 border-blue-400/20' : 'bg-emerald-400/10 text-emerald-400 border-emerald-400/20'}`}>
                    {h.field === 'planned' ? 'Planned Date' : 'Actual Date'}
                  </Badge>
                  <span className="text-[10px] text-neutral-500 font-mono">Rev #{h.revision}</span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                  <span className="text-neutral-500 line-through">{h.old_value || '(empty)'}</span>
                  <span className="text-neutral-600">→</span>
                  <span className="text-white font-medium">{h.new_value || '(empty)'}</span>
                </div>
                <div className="text-[10px] text-neutral-600 mt-1">
                  by {h.changed_by} · {h.changed_at ? new Date(h.changed_at).toLocaleString() : ''}
                </div>
              </div>
            ))}
          </div>

...
import { STATUS_LABELS, STATUS_COLORS, TYPE_COLORS, TIER_COLORS, CX_COLORS, calcProgress, getBlockers } from '@/lib/constants';
    ...
    <div data-testid="dashboard-page">
      ...
      <div className="px-8 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3" data-testid="project-grid">
          {filtered.map((p, i) => (
            <ProjectCard key={p.id} project={p} navigate={navigate} index={i} />
          ))}
          {filtered.length === 0 && (
            <div className="col-span-full text-center py-16" data-testid="empty-state">
              ...
              {!projects.length && (
                <button onClick={() => setShowModal(true)} className="mt-3 inline-flex items-center gap-1.5 rounded-md text-sm font-medium bg-white text-black hover:bg-neutral-200 h-9 px-4 transition-colors">
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-2 mt-1">
        ...
        <Badge className={`${sc.bg} ${sc.text} ${sc.border} text-[10px] font-semibold shrink-0`}>
      ...
      <div className="flex gap-3 flex-wrap mb-3 text-[11px] text-neutral-500">
        ...
        {p.launch && <span>Launch: <strong className="text-neutral-300">{p.launch}</strong></span>}
        {p.owner && <span><strong className="text-neutral-300">{p.owner}</strong></span>}
        {blockers > 0 && <span className="text-rose-400 flex items-center gap-1"><AlertTriangle className="w-3 h-3" />{blockers} blocker{blockers !== 1 ? 's' : ''}</span>}
        {delay > 0 && <span className="text-rose-400">+{delay}d delay</span>}
      </div>
